...
GENERATED_TEST_SERVER = GENERATED_TEST_ROOT / "test-server"
GENERATED_TEST_PORT = os.getenv("DD_TEST_SERVER_PORT", "18082")
GENERATED_TESTS_ENABLED = os.getenv("DD_USE_GENERATED_TESTS", "false").lower() == "true"
GENERATED_TEST_RUNNER_BANNER = "=== Using Generated Test Runner ==="

if GENERATED_TESTS_ENABLED and RECORD == "false" and GENERATED_TEST_SERVER.is_file():
    os.environ.setdefault("DD_TEST_RUNNER_DATA", str(GENERATED_TEST_ROOT / "test-runner-data"))
    os.environ.setdefault("DD_TEST_SERVER_URL", f"http://127.0.0.1:{GENERATED_TEST_PORT}")


def test_runner_enabled():
    """Return whether generated BDD request plans should drive the test."""
    return GENERATED_TESTS_ENABLED and "DD_TEST_RUNNER_DATA" in os.environ


def test_server_enabled():
    """Return whether generated recordings should be served over HTTP."""
    return GENERATED_TESTS_ENABLED and "DD_TEST_SERVER_URL" in os.environ


def test_features_root():
    """Return the feature-data root selected for this test run."""
    return pathlib.Path(os.path.dirname(__file__))


def test_feature_path(version, *parts):
    return test_features_root().joinpath(version, "features", *parts)


@pytest.fixture(scope="session", autouse=True)
def generated_test_server():
    """Run the generated replay server for the lifetime of the pytest session."""
    if not test_server_enabled():
        yield
        return
    print(GENERATED_TEST_RUNNER_BANNER, file=sys.__stdout__, flush=True)
    if not GENERATED_TEST_SERVER.is_file():
        yield
        return

    log_path = pathlib.Path(os.getenv("DD_TEST_SERVER_LOG", tempfile.gettempdir() + "/datadog-python-test-server.log"))
    with log_path.open("w") as server_log:
        process = subprocess.Popen(
            [str(GENERATED_TEST_SERVER), "--port", GENERATED_TEST_PORT],
            stdout=server_log,
            stderr=server_log,
        )
        health_url = os.environ["DD_TEST_SERVER_URL"].rstrip("/") + "/__openapi_transformer__/health"
        for _ in range(50):
            try:
                ...
                if process.poll() is not None:
                    raise RuntimeError(f"Generated test server exited early; see {log_path}")
                time.sleep(0.1)
        else:
            process.terminate()
            raise RuntimeError(f"Generated test server failed to start; see {log_path}")
...
def test_server_session(request, api_version):
    ...
    try:
        ...
        _test_server_request("POST", f"/__openapi_transformer__/sessions/{session['session']}/stop")
...
@pytest.fixture
def freezed_time(default_cassette_name, record_mode, request, test_server_session):
...
def undo_operations():
    ...
    for version in ("v1", "v2"):
        f = test_feature_path(version, "undo.json")
        with f.open() as fp:
            data = json.load(fp)
            result[version] = {}
...
def request_body_from_file(context, path, package_name):
    ...
    with test_feature_path(version, path).open() as f:
        data = f.read()
    tpl = Template(data).render(**context)
    context["api_request"]["kwargs"]["body"] = tpl
...
for version in ("v1", "v2"):
    f = test_feature_path(version, "given.json")
    with f.open() as fp:
...
def execute_request(undo, context, client, api_version, request, path_parameters):
    ...
    for k, v in api_request["kwargs"].items():
        ...
        if openapi_types == (file_type,):
            filepath = test_feature_path(api_version, json.loads(v))
