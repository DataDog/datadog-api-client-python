...
Feature: Logs
  ...
  Scenario: Aggregate events returns "OK" response
    ...
    And body {"compute": [{"aggregation": "count", "interval": "100", "metric": "test.aggregation.{{ unique }}", "type": "timeseries"}], "filter": {"from": "1600348573", "indexes": ["main"], "query": "datadog-agent", "to": "1600348600"}}
    When the request is sent
    Then the response status is 200 OK
