# Unless explicitly stated otherwise all files in this repository are licensed under the Apache-2.0 License.
# This product includes software developed at Datadog (https://www.datadoghq.com/).
# Copyright 2019-Present Datadog, Inc.


import sys
import unittest

import datadog_api_client.v2
from datadog_api_client.v2.model.logs_archive_destination import LogsArchiveDestination
from datadog_api_client.v2.model.logs_archive_state import LogsArchiveState

globals()["LogsArchiveDestination"] = LogsArchiveDestination
globals()["LogsArchiveState"] = LogsArchiveState
from datadog_api_client.v2.model.logs_archive_attributes import LogsArchiveAttributes


class TestLogsArchiveAttributes(unittest.TestCase):
    """LogsArchiveAttributes unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def testLogsArchiveAttributes(self):
        """Test LogsArchiveAttributes"""
        # FIXME: construct object with mandatory attributes with example values
        # model = LogsArchiveAttributes()  # noqa: E501
        pass


if __name__ == "__main__":
    unittest.main()
