# Unless explicitly stated otherwise all files in this repository are licensed under the Apache-2.0 License.
# This product includes software developed at Datadog (https://www.datadoghq.com/).
# Copyright 2019-Present Datadog, Inc.


import sys
import unittest

import datadog_api_client.v1
from datadog_api_client.v1.model.slack_integration_channel import SlackIntegrationChannel

globals()["SlackIntegrationChannel"] = SlackIntegrationChannel
from datadog_api_client.v1.model.slack_integration_channels import SlackIntegrationChannels


class TestSlackIntegrationChannels(unittest.TestCase):
    """SlackIntegrationChannels unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def testSlackIntegrationChannels(self):
        """Test SlackIntegrationChannels"""
        # FIXME: construct object with mandatory attributes with example values
        # model = SlackIntegrationChannels()  # noqa: E501
        pass


if __name__ == "__main__":
    unittest.main()
