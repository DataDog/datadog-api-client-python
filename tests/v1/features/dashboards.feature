...
Feature: Dashboards
  ...
  Scenario: Create a new dashboard with a formulas and functions treemap widget
    Given new "CreateDashboard" request
    And body with value { "title": "{{ unique }}", "widgets": [ { "definition": { "title": "", "type": "treemap", "requests": [ { "formulas": [ { "formula": "hour_before(query1)" }, { "formula": "query1" } ], "queries": [ { "data_source": "logs", "name": "query1", "search": { "query": "" }, "indexes": [ "*" ], "compute": { "aggregation": "count" } } ], "response_format": "scalar" } ] }, "layout": { "x": 0, "y": 0, "width": 4, "height": 4 } } ], "layout_type": "ordered" }
    When the request is sent
    Then the response status is 200 OK
    And the response "widgets[0].definition.requests[0].response_format" is equal to "scalar"
    And the response "widgets[0].definition.requests[0].queries[0].data_source" is equal to "logs"
  ...
  Scenario: Create a new dashboard with an audit logs query
    Given new "CreateDashboard" request
    And body with value {"layout_type": "ordered", "title": "{{ unique }} with Audit Logs Query", "widgets": [{"definition": {"type": "timeseries","requests": [{"response_format": "timeseries","queries": [{"search": {"query": ""},"data_source": "audit","compute": {"aggregation": "count"},"name": "query1","indexes": ["*"]}]}]},"layout": {"x": 2,"y": 0,"width": 4,"height": 2}}]}
    When the request is sent
    Then the response status is 200 OK
  ...
  @team:DataDog/dashboards-backend
  Scenario: Create a new dashboard with formula and function distribution widget
    Given new "CreateDashboard" request
    And body with value {"title": "{{ unique }}", "widgets": [{"layout": {"x": 0, "y": 0, "width": 47, "height": 15}, "definition": {"title": "", "title_size": "16", "title_align": "left", "time": {}, "type": "distribution", "requests": [{"response_format": "scalar", "queries": [{"data_source": "logs", "name": "query1", "search": {"query": ""}, "indexes": ["*"], "compute": {"aggregation": "avg", "metric": "@duration"}, "group_by": [{"facet": "service", "limit": 1000, "sort": {"aggregation": "count", "order": "desc"}}], "storage": "hot"}]}]}}], "template_variables": [], "layout_type": "free", "notify_list": []}
    When the request is sent
    Then the response status is 200 OK
    And the response "widgets[0].definition.type" is equal to "distribution"
    And the response "widgets[0].definition.requests[0].response_format" is equal to "scalar"
    And the response "widgets[0].definition.requests[0].queries[0].data_source" is equal to "logs"
    And the response "widgets[0].definition.requests[0].queries[0].name" is equal to "query1"
    And the response "widgets[0].definition.requests[0].queries[0].compute.aggregation" is equal to "avg"
    And the response "widgets[0].definition.requests[0].queries[0].compute.metric" is equal to "@duration"
    And the response "widgets[0].definition.requests[0].queries[0].group_by[0].facet" is equal to "service"
  ...
  Scenario: Create a new dashboard with formulas and functions events query using facet group by
    Given new "CreateDashboard" request
    And body with value {"title": "{{ unique }} with events facet group_by", "widgets": [{"definition": {"type": "timeseries", "requests": [{"response_format": "timeseries", "queries": [{"data_source": "events", "name": "query1", "search": {"query": ""}, "compute": {"aggregation": "count"}, "group_by": [{"facet": "service", "limit": 10}]}]}]}, "layout": {"x": 0, "y": 0, "width": 4, "height": 2}}], "layout_type": "ordered"}
    When the request is sent
    Then the response status is 200 OK
    And the response "widgets[0].definition.requests[0].queries[0].data_source" is equal to "events"
    And the response "widgets[0].definition.requests[0].queries[0].group_by[0].facet" is equal to "service"
    And the response "widgets[0].definition.requests[0].queries[0].group_by[0].limit" is equal to 10

  @team:DataDog/dashboards-backend
  Scenario: Create a new dashboard with formulas and functions events query using flat group by fields
    Given new "CreateDashboard" request
    And body with value {"title": "{{ unique }} with events flat group_by fields", "widgets": [{"definition": {"type": "timeseries", "requests": [{"response_format": "timeseries", "queries": [{"data_source": "events", "name": "query1", "search": {"query": ""}, "compute": {"aggregation": "count"}, "group_by": {"fields": ["service", "host"], "limit": 10}}]}]}, "layout": {"x": 0, "y": 0, "width": 4, "height": 2}}], "layout_type": "ordered"}
    When the request is sent
    Then the response status is 200 OK
    And the response "widgets[0].definition.requests[0].queries[0].data_source" is equal to "events"
    And the response "widgets[0].definition.requests[0].queries[0].group_by.fields[0]" is equal to "service"
    And the response "widgets[0].definition.requests[0].queries[0].group_by.fields[1]" is equal to "host"
    And the response "widgets[0].definition.requests[0].queries[0].group_by.limit" is equal to 10
  ...
  Scenario: Create a new dashboard with hostmap infra widget
    ...
    Then the response status is 200 OK
    And the response "widgets[0].definition.type" is equal to "hostmap"
    And the response "widgets[0].definition.requests" is equal to {"request_type": "infrastructure_hostmap", "node_type": "host", "filter": "env:prod", "group_by": [{"column": "tags", "key": "service"}], "enrichments": [{"response_format": "scalar", "queries": [{"data_source": "metrics", "name": "query1", "query": "avg:system.cpu.user{*} by {host}"}], "formulas": [{"formula": "query1", "dimension": "fill"}]}], "style": {"palette": "green_to_orange", "palette_flip": false}}
  ...
  Scenario: Create a new dashboard with sankey widget and network data source
    ...
    Then the response status is 200 OK
    And the response "widgets[0].definition.type" is equal to "sankey"
    And the response "widgets[0].definition.requests[0].query.data_source" is equal to "network"
    And the response "widgets[0].definition.requests[0].query.query_string" is equal to "*"
    And the response "widgets[0].definition.requests[0].query.group_by" is equal to ["source", "destination"]
    And the response "widgets[0].definition.requests[0].query.limit" is equal to 100
    And the response "widgets[0].definition.requests[0].request_type" is equal to "netflow_sankey"

  @team:DataDog/dashboards-backend
  Scenario: Create a new dashboard with sankey widget and product analytics data source
    Given new "CreateDashboard" request
    And body from file "dashboards_json_payload/sankey_product_analytics_widget.json"
    When the request is sent
    Then the response status is 200 OK
    And the response "widgets[0].definition.type" is equal to "sankey"
