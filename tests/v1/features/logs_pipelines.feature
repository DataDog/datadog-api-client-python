...
Feature: Logs Pipelines
  ...
  @generated @skip @team:DataDog/logs-onboarding
  ...
  @team:DataDog/logs-onboarding
  Scenario: Create a pipeline with Array Map Processor with preserve_source false and same source and target returns "Bad Request" response
    Given new "CreateLogsPipeline" request
    And body with value {"filter": {"query": "source:python"}, "name": "testPipelineArrayMapInvalid", "processors": [{"type": "array-map-processor", "is_enabled": true, "name": "invalid preserve_source", "source": "items", "target": "items", "preserve_source": false, "processors": [{"type": "attribute-remapper", "sources": ["$sourceElem.id"], "target": "$targetElem.uid"}]}], "tags": []}
    When the request is sent
    Then the response status is 400 Bad Request
  ...
  @team:DataDog/logs-onboarding
  Scenario: Create a pipeline with Array Map Processor with targetElem used as source returns "Bad Request" response
    Given new "CreateLogsPipeline" request
    And body with value {"filter": {"query": "source:python"}, "name": "testPipelineArrayMapTargetElemSource", "processors": [{"type": "array-map-processor", "is_enabled": true, "name": "invalid targetElem source", "source": "items", "target": "out", "processors": [{"type": "attribute-remapper", "sources": ["$targetElem.uid"], "target": "$targetElem.label"}]}], "tags": []}
    When the request is sent
    Then the response status is 400 Bad Request
