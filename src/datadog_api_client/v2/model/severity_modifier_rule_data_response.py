# Unless explicitly stated otherwise all files in this repository are licensed under the Apache-2.0 License.
# This product includes software developed at Datadog (https://www.datadoghq.com/).
# Copyright 2019-Present Datadog, Inc.
from __future__ import annotations

from typing import TYPE_CHECKING

from datadog_api_client.model_utils import (
    ModelNormal,
    cached_property,
    UUID,
)


if TYPE_CHECKING:
    from datadog_api_client.v2.model.severity_modifier_rule_attributes_response import (
        SeverityModifierRuleAttributesResponse,
    )
    from datadog_api_client.v2.model.severity_modifier_rule_type import SeverityModifierRuleType


class SeverityModifierRuleDataResponse(ModelNormal):
    @cached_property
    def openapi_types(_):
        from datadog_api_client.v2.model.severity_modifier_rule_attributes_response import (
            SeverityModifierRuleAttributesResponse,
        )
        from datadog_api_client.v2.model.severity_modifier_rule_type import SeverityModifierRuleType

        return {
            "attributes": (SeverityModifierRuleAttributesResponse,),
            "id": (UUID,),
            "type": (SeverityModifierRuleType,),
        }

    attribute_map = {
        "attributes": "attributes",
        "id": "id",
        "type": "type",
    }

    def __init__(
        self_, attributes: SeverityModifierRuleAttributesResponse, id: UUID, type: SeverityModifierRuleType, **kwargs
    ):
        """
        The data object for a severity modifier rule as returned by the API.

        :param attributes: Attributes of a severity modifier rule as returned by the API.
        :type attributes: SeverityModifierRuleAttributesResponse

        :param id: The ID of the severity modifier rule.
        :type id: UUID

        :param type: The JSON:API type for severity modifier rules.
        :type type: SeverityModifierRuleType
        """
        super().__init__(kwargs)

        self_.attributes = attributes
        self_.id = id
        self_.type = type
