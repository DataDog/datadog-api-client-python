# Unless explicitly stated otherwise all files in this repository are licensed under the Apache-2.0 License.
# This product includes software developed at Datadog (https://www.datadoghq.com/).
# Copyright 2019-Present Datadog, Inc.
from __future__ import annotations

from typing import TYPE_CHECKING

from datadog_api_client.model_utils import (
    ModelNormal,
    cached_property,
)


if TYPE_CHECKING:
    from datadog_api_client.v2.model.test_optimization_get_service_settings_request_data import (
        TestOptimizationGetServiceSettingsRequestData,
    )


class TestOptimizationGetServiceSettingsRequest(ModelNormal):
    @cached_property
    def openapi_types(_):
        from datadog_api_client.v2.model.test_optimization_get_service_settings_request_data import (
            TestOptimizationGetServiceSettingsRequestData,
        )

        return {
            "data": (TestOptimizationGetServiceSettingsRequestData,),
        }

    attribute_map = {
        "data": "data",
    }

    def __init__(self_, data: TestOptimizationGetServiceSettingsRequestData, **kwargs):
        """
        Request object for getting Test Optimization service settings.

        :param data: Data object for get service settings request.
        :type data: TestOptimizationGetServiceSettingsRequestData
        """
        super().__init__(kwargs)

        self_.data = data
