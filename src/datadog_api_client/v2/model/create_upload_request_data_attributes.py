...
class CreateUploadRequestDataAttributes(ModelNormal):
    ...
    def __init__(self_, headers: List[str], part_count: int, part_size: int, table_name: str, **kwargs):
        ...
        :param part_size: The size of each part in the upload in bytes. For multi-part uploads (part_count > 1), all parts except the last one must be at least 5,000,000 bytes. For single-part uploads (part_count = 1), any size is allowed.
        :type part_size: int
