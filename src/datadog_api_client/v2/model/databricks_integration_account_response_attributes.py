...
if TYPE_CHECKING:
    ...
    from datadog_api_client.v2.model.databricks_integration_account_private_action_runner_auth_response import (
        DatabricksIntegrationAccountPrivateActionRunnerAuthResponse,
    )
    from datadog_api_client.v2.model.databricks_integration_account_bearer_token_auth_response import (
...
class DatabricksIntegrationAccountResponseAttributes(ModelNormal):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        :param authentication: Authentication configured on the Databricks integration account. A ``bearer_token`` method indicates an account still on token authentication, which Databricks accepts only on accounts that already use it.
        :type authentication: DatabricksIntegrationAccountAuthenticationResponse, optional

        :param dataflows: Data Datadog collects from Databricks, keyed by dataflow id.
