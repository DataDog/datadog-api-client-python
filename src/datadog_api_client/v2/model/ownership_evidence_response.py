...
class OwnershipEvidenceResponse(ModelNormal):
    @cached_property
    def openapi_types(_):
        from datadog_api_client.v2.model.ownership_evidence_data import OwnershipEvidenceData
    ...
    def __init__(self_, data: OwnershipEvidenceData, **kwargs):
        """
        The response returned when retrieving the evidence backing an ownership inference for an owner type.

        :param data: The data wrapper for an ownership evidence response.
        :type data: OwnershipEvidenceData
        """
