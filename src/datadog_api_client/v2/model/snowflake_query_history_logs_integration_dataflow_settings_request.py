...
class SnowflakeQueryHistoryLogsIntegrationDataflowSettingsRequest(ModelNormal):
    @cached_property
    ...
    def __init__(
        ...
        **kwargs,
        ...
        :param join_query_history_with_access_history_enabled: Whether query logs are joined with Snowflake access history, which adds the objects each query read and wrote so you can follow how data is used and where it came from. Defaults to ``false``.
        :type join_query_history_with_access_history_enabled: bool, optional

        :param query_history_logs_interval_min: How often query history logs are collected, in minutes. One of ``5`` , ``15`` , ``30`` , ``60`` , ``720`` , or ``1440``. Defaults to ``5``.
