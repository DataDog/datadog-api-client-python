...
class DatabricksIntegrationAccountSettingsUpdate(ModelNormal):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        :param system_tables_sql_warehouse_id: ID of the SQL warehouse used to query the Databricks system tables. Find it at the end of the HTTP Path under Warehouses > Connection Details in the Databricks console. Required once the cloud cost metrics or the Data Observability dataflow is enabled, or while ``serverless_jobs_enabled`` is on; an update that leaves the account without one is rejected with a ``422`` response.
