# Unless explicitly stated otherwise all files in this repository are licensed under the Apache-2.0 License.
# This product includes software developed at Datadog (https://www.datadoghq.com/).
# Copyright 2019-Present Datadog, Inc.
from __future__ import annotations

from typing import TYPE_CHECKING

from datadog_api_client.model_utils import (
    ModelNormal,
    cached_property,
)


if TYPE_CHECKING:
    from datadog_api_client.v2.model.terraform_backend_update_attributes import TerraformBackendUpdateAttributes
    from datadog_api_client.v2.model.terraform_backend_type import TerraformBackendType


class TerraformBackendUpdateData(ModelNormal):
    @cached_property
    def openapi_types(_):
        from datadog_api_client.v2.model.terraform_backend_update_attributes import TerraformBackendUpdateAttributes
        from datadog_api_client.v2.model.terraform_backend_type import TerraformBackendType

        return {
            "attributes": (TerraformBackendUpdateAttributes,),
            "id": (str,),
            "type": (TerraformBackendType,),
        }

    attribute_map = {
        "attributes": "attributes",
        "id": "id",
        "type": "type",
    }

    def __init__(self_, attributes: TerraformBackendUpdateAttributes, id: str, type: TerraformBackendType, **kwargs):
        """
        Terraform backend configuration update data.

        :param attributes: Replacement bucket set for a Terraform backend sync configuration.
        :type attributes: TerraformBackendUpdateAttributes

        :param id: Configuration ID. Must match the ID in the request path.
        :type id: str

        :param type: Terraform backend configuration resource type.
        :type type: TerraformBackendType
        """
        super().__init__(kwargs)

        self_.attributes = attributes
        self_.id = id
        self_.type = type
