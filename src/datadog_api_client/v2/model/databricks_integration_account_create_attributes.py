...
if TYPE_CHECKING:
    ...
    from datadog_api_client.v2.model.databricks_integration_account_private_action_runner_auth_request import (
        DatabricksIntegrationAccountPrivateActionRunnerAuthRequest,
    )
    from datadog_api_client.v2.model.databricks_integration_account_bearer_token_auth_request import (
...
class DatabricksIntegrationAccountCreateAttributes(ModelNormal):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        :param authentication: Authentication for creating the Databricks integration account. Exactly one method is set. Choosing ``private_action_runner`` leaves the ``databricks-model-serving-metrics`` dataflow unable to collect data. ``bearer_token`` is listed but always rejected on creation: Databricks accepts it only on accounts that already use it.
