# Unless explicitly stated otherwise all files in this repository are licensed under the Apache-2.0 License.
# This product includes software developed at Datadog (https://www.datadoghq.com/).
# Copyright 2019-Present Datadog, Inc.
from __future__ import annotations


from datadog_api_client.model_utils import (
    ModelSimple,
    cached_property,
)

from typing import ClassVar


class LLMObsPromptVersionType(ModelSimple):
    """
    Resource type of an LLM Observability prompt version.

    :param value: If omitted defaults to "prompt-template-versions". Must be one of ["prompt-template-versions"].
    :type value: str
    """

    allowed_values = {
        "prompt-template-versions",
    }
    PROMPT_TEMPLATE_VERSIONS: ClassVar["LLMObsPromptVersionType"]

    @cached_property
    def openapi_types(_):
        return {
            "value": (str,),
        }


LLMObsPromptVersionType.PROMPT_TEMPLATE_VERSIONS = LLMObsPromptVersionType("prompt-template-versions")
