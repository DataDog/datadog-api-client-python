...
class DatabricksDataObservabilityQualityMonitoringIntegrationDataflowSettingsResponse(ModelNormal):
    @cached_property
    def openapi_types(_):
        return {
    ...
    def __init__(
        ...
        **kwargs,
        ...
        :param do_crawlers_cron: Cron expression setting how often Datadog connects to your Databricks warehouse to collect metadata. Currently, only hourly ( ``0 * * * *`` ) and daily ( ``0 0 * * *`` ) are supported.
        :type do_crawlers_cron: str, optional

        :param sync_system_catalog: Whether metadata from the Databricks ``system`` catalog is included in Data Observability alongside your data catalogs.
