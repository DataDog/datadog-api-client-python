...
class ElasticCloudMonitoringAccountCreateData(ModelNormal):
    @cached_property
    def openapi_types(_):
        from datadog_api_client.v2.model.elastic_cloud_monitoring_account_attributes import (
            ElasticCloudMonitoringAccountAttributes,
    ...
    def __init__(self_, attributes: ElasticCloudMonitoringAccountAttributes, type: IntegrationAccountType, **kwargs):
        """
        Data envelope for creating an Elastic Cloud monitoring account.

        :param attributes: Attributes of an Elastic Cloud monitoring account. The configuration is hoisted directly onto the attributes; there is no interface wrapper because the ``elastic-cloud`` interface is fixed by the endpoint path.
        :type attributes: ElasticCloudMonitoringAccountAttributes
