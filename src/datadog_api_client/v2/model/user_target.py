# Unless explicitly stated otherwise all files in this repository are licensed under the Apache-2.0 License.
# This product includes software developed at Datadog (https://www.datadoghq.com/).
# Copyright 2019-Present Datadog, Inc.
from __future__ import annotations

from typing import TYPE_CHECKING

from datadog_api_client.model_utils import (
    ModelNormal,
    cached_property,
)


if TYPE_CHECKING:
    from datadog_api_client.v2.model.user_target_type import UserTargetType


class UserTarget(ModelNormal):
    @cached_property
    def openapi_types(_):
        from datadog_api_client.v2.model.user_target_type import UserTargetType

        return {
            "id": (str,),
            "type": (UserTargetType,),
        }

    attribute_map = {
        "id": "id",
        "type": "type",
    }

    def __init__(self_, id: str, type: UserTargetType, **kwargs):
        """
        Represents a user target for an escalation policy step, including the user's ID and resource type.

        :param id: Specifies the unique identifier of the user resource.
        :type id: str

        :param type: Indicates that the resource is of type ``users``.
        :type type: UserTargetType
        """
        super().__init__(kwargs)

        self_.id = id
        self_.type = type
