...
class RoutingRuleEscalationPolicyAction(ModelNormal):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        :param type: Indicates that the action routes to an escalation policy. This action can be set once per routing rule, and cannot be combined with the legacy top-level ``policy_id`` field on the routing rule.
