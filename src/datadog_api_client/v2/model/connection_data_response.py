# Unless explicitly stated otherwise all files in this repository are licensed under the Apache-2.0 License.
# This product includes software developed at Datadog (https://www.datadoghq.com/).
# Copyright 2019-Present Datadog, Inc.
from __future__ import annotations

from typing import TYPE_CHECKING

from datadog_api_client.model_utils import (
    ModelNormal,
    cached_property,
)


if TYPE_CHECKING:
    from datadog_api_client.v2.model.connection_data_attributes_response import ConnectionDataAttributesResponse
    from datadog_api_client.v2.model.action_connection_data_type import ActionConnectionDataType


class ConnectionDataResponse(ModelNormal):
    @cached_property
    def openapi_types(_):
        from datadog_api_client.v2.model.connection_data_attributes_response import ConnectionDataAttributesResponse
        from datadog_api_client.v2.model.action_connection_data_type import ActionConnectionDataType

        return {
            "attributes": (ConnectionDataAttributesResponse,),
            "id": (str,),
            "type": (ActionConnectionDataType,),
        }

    attribute_map = {
        "attributes": "attributes",
        "id": "id",
        "type": "type",
    }

    def __init__(
        self_, attributes: ConnectionDataAttributesResponse, id: str, type: ActionConnectionDataType, **kwargs
    ):
        """
        Data object for an action connection.

        :param attributes: Attributes of an action connection.
        :type attributes: ConnectionDataAttributesResponse

        :param id: The unique identifier of the connection.
        :type id: str

        :param type: The definition of ``ActionConnectionDataType`` object.
        :type type: ActionConnectionDataType
        """
        super().__init__(kwargs)

        self_.attributes = attributes
        self_.id = id
        self_.type = type
