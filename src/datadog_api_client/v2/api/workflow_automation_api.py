...
class WorkflowAutomationApi:
    ...
    def list_workflows(
        ...
        *,
        ...
        filter_trigger_type: Union[List[str], UnsetType] = unset,
        ...
        :param filter_query: A search query used to filter the returned workflows. The query performs a case-insensitive substring match against each workflow's name, creator name, and handle. If the query contains a colon (for example, ``team:infra`` ), it is instead treated as a ``key:value`` tag filter.
        :type filter_query: str, optional
        :param filter_trigger_type: Filter the returned workflows by one or more trigger types, such as ``monitor`` , ``schedule`` , or ``githubWebhook``. Repeat the parameter to filter by multiple trigger types.
        :type filter_trigger_type: [str], optional
        ...
        if filter_trigger_type is not unset:
            kwargs["filter_trigger_type"] = filter_trigger_type
