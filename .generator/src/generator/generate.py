...
def temp_generate(specs):
    tempdir = tempfile.TemporaryDirectory()

    generate(specs, pathlib.Path(tempdir.name))

    with (pathlib.Path(tempdir.name) / PACKAGE_NAME / "version.py").open("w") as f:
        f.write("__version__ = \"0.0.1.local\"\n")
...
def generate(specs, output):
    ...
    with filename.open("w") as fp:
        fp.write(configuration_j2.render(spec=spec, apis=all_apis))
